...
void BitcoinExchange::exchange(const char * arg, int min_year)
{
    ...
    while (std::getline(file, databaseFileLine))
    {
        ...
        int i = 0;
        while(databaseFileLine[i] != '|')
        {
            if(databaseFileLine[i] <= '0' && databaseFileLine[i] >= '9')
            {
                control = 0;
                break ;
            }
        ...
        while(databaseFileLine[i])
        {
            if((databaseFileLine[i] >= '0' && databaseFileLine[i] <= '9') || databaseFileLine[i] == '.')
                i++;
            else
            {
                control = 0;
        ...
        if(databaseFileLine.size() > 11 && control)
        {
